...
func parseFlags() {
	...
	flag.BoolVar(&flagV2EnableUploadReaper, "v2-enable-upload-reaper", false, "Enable upload reaper")
	flag.Parse()
...
func main() {
	log.Printf("Starting Homewatch version %s", softwareVersion)
	parseFlags()
	if flagDebug {
		debugFlags()
	}
